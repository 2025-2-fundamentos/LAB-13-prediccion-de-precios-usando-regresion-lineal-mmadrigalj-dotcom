...
def age_feature(df: pd.DataFrame) -> pd.DataFrame:
    data_aux = df.copy()
    data_aux["Age"] = 2021 - data_aux["Year"]
    data_aux.drop(columns=["Year", "Car_Name"], inplace=True)
    return data_aux
...
def preprocessing_pipeline(x_train: pd.DataFrame) -> Pipeline:
...
def optimize_cross_validation(pipeline, x_train, y_train):
...
def clean_model_directory(output_directory):
...
if __name__ == "__main__":
    ...
    X_train = age_feature(X_train)
    X_test = age_feature(X_test)

    pipeline = preprocessing_pipeline(X_train)
    best_estimator = optimize_cross_validation(pipeline, X_train, y_train)
